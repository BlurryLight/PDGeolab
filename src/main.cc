...
int main()
{
  ...
  std::fstream fs;
  std::string vshader;
  PD::ResourcePathSearcher ResManager;
  ResManager.add_path(PD::ResourcePathSearcher::root_path / "shaders");
  fs.open(ResManager.find_path("simple.vert"));
  if (fs.good()) {
    std::stringstream ss;
    ss << fs.rdbuf();
    vshader = ss.str();
  ...
  std::string fshader;
  fs.open(ResManager.find_path("simple.frag"));
  if (fs.good()) {
    std::stringstream ss;
    ss << fs.rdbuf();
    fshader = ss.str();
